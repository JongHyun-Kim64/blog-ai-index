...
assert.equal(tip.callbacks.title([{dataIndex:0}]),'9월 12일 (토)');
assert.equal(tip.callbacks.title([{dataIndex:1}]),'9월 13일 (일)');
assert.equal(tip.callbacks.title([{dataIndex:99}]),'날짜 정보 없음');
assert.equal(tip.callbacks.label({formattedValue:'0'}),'방문자 0');
assert.equal(tip.callbacks.label({formattedValue:'1,234'}),'방문자 1,234');
...
assert.equal(tip.callbacks.title([{dataIndex:1}]),'10월 1일 (목)');
