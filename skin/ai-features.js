...
(function () {
  ...
  var CONFIG = {
    ...
    WORKER_URL: "",
    RELATED_COUNT: 5,
    // 우하단 "맨 위로" 버튼(46px, bottom 18px) 위에 세로 정렬로 배치
    FAB_BOTTOM: 78,
    FAB_RIGHT: 22
  };
...
".aiblog-fab{position:fixed;right:" + CONFIG.FAB_RIGHT + "px;bottom:" + CONFIG.FAB_BOTTOM + "px;z-index:9998;height:44px;padding:0 15px 0 13px;display:flex;align-items:center;gap:7px;border-radius:22px;border:none;background:var(--accent,#2b2f36);color:#fff;font-size:13.5px;font-weight:800;letter-spacing:.4px;font-family:inherit;cursor:pointer;box-shadow:0 8px 24px rgba(0,23,51,.18);transition:transform .16s ease,box-shadow .16s ease}" +
".aiblog-fab:hover{transform:translateY(-2px);box-shadow:0 12px 28px rgba(0,23,51,.24)}" +
".aiblog-fab:active{transform:translateY(0)}" +
/* --- 어시스턴트 패널 --- */
".aiblog-wrap{position:fixed;right:24px;bottom:134px;z-index:9999;width:376px;max-width:calc(100vw - 32px);opacity:0;pointer-events:none;transform:translateY(10px) scale(.98);transition:opacity .18s ease,transform .18s ease}" +
...
".aiblog-fab{right:16px;bottom:76px}" +
".aiblog-wrap{right:12px;left:12px;bottom:130px;width:auto}" +
...
"}";
  ...
  function buildAssistant(index, curPost, byId) {
    ...
    function replySearch(q) {
      ...
      if (!items.length) {
        aiBubble("“" + esc(q) + "”에 대한 글을 찾지 못했어요. 다른 키워드로 검색해보세요.");
        return;
      }
    ...
    function route(q) {
      ...
      if (it.kind === "summary") replySummary(it.topic);
    ...
    function openPanel() {
      ...
      if (!greeted) {
        ...
        var hello = curPost
          ...
          : "안녕하세요! 이 블로그의 글 " + posts.length + "개를 학습했어요.<br>궁금한 주제를 검색하거나 “최근 글”을 눌러보세요.";
        aiBubble(hello);
      }
